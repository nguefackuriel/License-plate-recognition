...
with header :
    ...
    col1.title("💥 Application de reconnaissance de plaques d'immatriculation 🚗")

    _, col0, _ = st.columns([0.15,1,0.1])
    col0.image("./imgs/voiture.png", width=500)
...
with body :
    ...
    if colb1.button("Televerser une image", on_click=change_state_uploader) :
        pass
    elif colb2.button("Prendre une Photo", on_click=change_state_camera) :
        pass
    elif colb3.button("Detection en Live ", on_click=change_state_live) :
        pass

    if st.session_state["state"] == "Uploader" :
        img = st.file_uploader("Televerser une image de voiture: ", type=["png", "jpg", "jpeg"])
    ...
    if img is not None:
        ...
        if col5.button("Appliquer la Detection"):
            ...
            if len(results) == 3 :
                ...
                if len(texts) == 1 and len(license_plate_crop) :
                    ...
                    for i in range(0, len(license_plate_crop)) :
                        ...
                        col11.success(f"Numero plaque {i}: {texts[i]}")
